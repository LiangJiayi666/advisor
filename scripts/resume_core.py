"""Resume core and harness utilities for Advisor.

This module contains the deterministic resume core: evidence loading,
job/evidence matching, draft construction, rewritten-bullet merge checks,
and harness validation.

It is intended to be imported by user-facing pipelines such as
``scripts.generate_tailored_resumes`` and by thin compatibility wrappers.
"""
...
def prepare_resume_for_job_card(
    job_card: Dict[str, Any],
    evidence_path: str | Path,
) -> Dict[str, Any]:
    """Prepare resume inputs from an already-normalized job card."""
    evidence_store = EvidenceStore(evidence_path)
    jd_parsed = parse_job_card(job_card)
    match_plan = match_evidence_to_job(jd_parsed, evidence_store)
    draft = draft_resume(jd_parsed, match_plan)
...
def validate_resume_for_job_card(
    job_card: Dict[str, Any],
    rewritten_bullets: List[Dict[str, Any]],
    evidence_path: str | Path,
    writer_name: str = "llm",
    advisor_data_dir: str | Path | None = None,
) -> Dict[str, Any]:
    """Validate rewritten bullets for a provided job card.

    When advisor_data_dir is provided, write the same output bundle as the store-based
    workflow. Otherwise return the validated structures in-memory.
    """
    prepared = prepare_resume_for_job_card(job_card, evidence_path)
    merged_bullets = _merge_rewritten_bullets(prepared["draft"]["bullets"], rewritten_bullets, prepared["match_plan"])
    draft = {**prepared["draft"], "bullets": merged_bullets, "writer": writer_name}
    harness_report = ResumeHarness(prepared["evidence_store"]).validate(prepared["jd_parsed"], draft)
    if advisor_data_dir is not None:
        output_job_id = (
            str(prepared["job_id"] or "").strip()
            or str(job_card.get("job_id") or "").strip()
            or str(job_card.get("normalized_job_id") or "").strip()
            or "job_card"
        )
        return _write_output(
            Path(advisor_data_dir),
            output_job_id,
            prepared["jd_parsed"],
            prepared["match_plan"],
            draft,
    ...
    return {
        ...
        "match_plan": prepared["match_plan"],
        "resume_draft": draft,
...
def _write_output(
    ...
    output_dir = advisor_data_dir / "resume" / "runs" / f"{job_id}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
