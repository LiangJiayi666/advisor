...
Compare two or more stored job cards from `advisor_data/jobs/jobs.jsonl`.

Workflow:
1. Resolve job IDs from the user's message. If the user refers to jobs by company/title instead of ID, search `advisor_data/jobs/jobs.jsonl` and show the matched candidates.
...
Do not compare by free-form memory only. Job postings are business-object data and must come from `advisor_data/jobs/`.
